...
export class ClassCreateDogForm extends Component<CreateDogFormProps, ClassCreateDogFormState> {
  ...
  render() {
    ...
    return (
      ...
      <input id="name" type="text" value={nameInput} onChange={(e) => this.setState({nameInput: e.target.value})} disabled={isLoading} />

      <label htmlFor="description">Dog Description</label>
      <textarea name="" id="description" value={descriptionInput} onChange={(e) => this.setState({descriptionInput: e.target.value})} cols={80} rows={10} disabled={isLoading}></textarea>

      <label htmlFor="picture">Select an Image</label>
      <select id="" onChange={(e) => this.setState({dogImage: e.target.value})}>
